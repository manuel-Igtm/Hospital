...
from .models import TestType, LabOrder, LabResult
...
class LabOrderAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'order_number', 'patient', 'test_type', 'ordering_provider',
        'priority', 'status', 'ordered_at'
    ]
    ...
    fieldsets = (
        ('Order Information', {
            'fields': ('id', 'order_number', 'patient', 'test_type', 'ordering_provider')
        }),
        ('Details', {
            'fields': ('priority', 'clinical_notes', 'status')
        }),
        ('Specimen', {
            'fields': ('specimen_collected_at', 'specimen_collected_by')
        }),
        ('Timestamps', {
...
class LabResultAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'order', 'is_abnormal', 'is_critical',
        'resulted_by', 'resulted_at', 'reviewed_by', 'reviewed_at'
    ]

...
class TestType(models.Model):
    ...
    code = models.CharField(
        ...
        unique=True,
        db_index=True,
        help_text='Test code (e.g., CBC, BMP, UA)'
    )
    ...
    loinc_code = models.CharField(
        max_length=20,
        blank=True,
        help_text='LOINC code for standardization'
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text='Whether this test is currently offered'
    )
...
class LabOrder(models.Model):
    ...
    clinical_notes = models.TextField(
        blank=True,
        help_text='Clinical indication/reason for test'
    )
    ...
    specimen_collected_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When specimen was collected'
    )
...
class LabResult(models.Model):
    ...
    hl7_obr_segment = models.TextField(
        blank=True,
        help_text='HL7 OBR (Observation Request) segment'
    )
    hl7_obx_segments = models.TextField(
        blank=True,
        help_text='HL7 OBX (Observation Result) segments, one per line'
    )
    
    # Parsed result summary
    result_summary = models.TextField(
        blank=True,
        help_text='Human-readable result summary'
    )
    
    # Flags
    is_abnormal = models.BooleanField(
        default=False,
        help_text='Whether any results are outside normal range'
    )
    is_critical = models.BooleanField(
        default=False,
        help_text='Whether results require immediate attention'
    )
    ...
    resulted_at = models.DateTimeField(
        auto_now_add=True,
        help_text='When results were entered'
    )
    ...
    reviewed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text='When results were reviewed'
    )
    review_notes = models.TextField(
        blank=True,
        help_text='Notes from reviewing physician'
    )
        ...
        if self.hl7_obr_segment:
            try:
                ...
                if not self.hl7_obr_segment.startswith('OBR|'):
                    raise ValidationError({
                        'hl7_obr_segment': 'Segment must be an OBR segment'
                    })
            except ValueError as e:
                raise ValidationError({
                    'hl7_obr_segment': f'Invalid HL7 OBR segment: {e}'
                })
            ...
            for i, segment in enumerate(self.hl7_obx_segments.strip().split('\n')):
                ...
                try:
                    ...
                    if not segment.startswith('OBX|'):
                        raise ValidationError({
                            'hl7_obx_segments': f'Line {i+1}: Segment must be an OBX segment'
                        })
                except ValueError as e:
                    raise ValidationError({
                        'hl7_obx_segments': f'Line {i+1}: Invalid HL7 OBX segment: {e}'
                    })

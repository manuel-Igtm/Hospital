...
class TestTypeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'loinc_code', 'specimen_type', 'turnaround_hours', 'is_active'
        ]
        read_only_fields = ['id']
...
class LabOrderListSerializer(serializers.ModelSerializer):
    ...
    ordering_provider_name = serializers.CharField(
        source='ordering_provider.full_name', read_only=True
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            'ordering_provider', 'ordering_provider_name',
            'priority', 'status', 'has_result', 'ordered_at'
        ]
...
class LabOrderDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'specimen_collected_at', 'specimen_collected_by',
...
class LabResultSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'order',
            'hl7_obr_segment', 'hl7_obx_segments',
            'result_summary', 'is_abnormal', 'is_critical',
            'resulted_by', 'resulted_at',
            'reviewed_by', 'reviewed_at', 'review_notes',
            'parsed_results'
        ]
        read_only_fields = [
            'id', 'resulted_by', 'resulted_at',
...
class LabResultCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LabResult
        fields = [
            'order', 'hl7_obr_segment', 'hl7_obx_segments',
            'result_summary', 'is_abnormal', 'is_critical'
        ]
    ...
    def validate_order(self, value):
        ...
        if value.status not in [OrderStatus.COLLECTED, OrderStatus.IN_PROGRESS]:
            raise serializers.ValidationError(
                'Can only enter results for collected or in-progress orders.'
            )

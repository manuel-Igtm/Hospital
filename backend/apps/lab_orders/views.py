...
from .models import LabOrder, LabResult, OrderStatus, TestType
...
class TestTypeViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        test_type.save()
        return Response({"message": f"Test type {test_type.code} has been deactivated."}, status=status.HTTP_200_OK)
...
class LabOrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_doctor:
            ...
            queryset = queryset.exclude(status__in=[OrderStatus.CANCELLED, OrderStatus.REVIEWED])

        return queryset

    @action(detail=True, methods=["post"], permission_classes=[IsClinicalStaff])
    @extend_schema(
        tags=["lab-orders"], request=LabOrderStatusUpdateSerializer, responses={200: LabOrderDetailSerializer}
    )
    def update_status(self, request, pk=None):
        """Update the order status with validation."""
        order = self.get_object()
        serializer = LabOrderStatusUpdateSerializer(data=request.data, context={"order": order, "request": request})
    ...
    def cancel(self, request, pk=None):
        ...
        if not order.can_transition_to(OrderStatus.CANCELLED):
            return Response(
                {"error": f"Cannot cancel order in {order.status} status"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def pending_results(self, request):
        ...
        orders = LabOrder.objects.filter(status__in=[OrderStatus.COLLECTED, OrderStatus.IN_PROGRESS]).select_related(
            "patient", "test_type", "ordering_provider"
        )
        serializer = LabOrderListSerializer(orders, many=True)
        return Response(serializer.data)
    ...
    def statistics(self, request):
        ...
        status_breakdown = dict(
            LabOrder.objects.values("status").annotate(count=Count("id")).values_list("status", "count")
        )

        priority_breakdown = dict(
            LabOrder.objects.values("priority").annotate(count=Count("id")).values_list("priority", "count")
        )
...
class LabResultViewSet(viewsets.ModelViewSet):
    ...
    queryset = LabResult.objects.select_related(
        "order", "order__patient", "order__test_type", "resulted_by", "reviewed_by"
    ).all()
    ...
    @action(detail=True, methods=["post"], permission_classes=[IsAdminOrDoctor])
    @extend_schema(tags=["lab-orders"], request=LabResultReviewSerializer, responses={200: LabResultSerializer})
    def review(self, request, pk=None):
        """Review and approve lab results."""
        result = self.get_object()

        if result.reviewed_by:
            return Response({"error": "Results have already been reviewed"}, status=status.HTTP_400_BAD_REQUEST)

        serializer = LabResultReviewSerializer(result, data=request.data, context={"request": request})
        serializer.is_valid(raise_exception=True)
        serializer.save()
    ...
    def pending_review(self, request):
        ...
        results = LabResult.objects.filter(reviewed_by__isnull=True).select_related(
    ...
    def critical(self, request):
        ...
        results = LabResult.objects.filter(is_critical=True, reviewed_by__isnull=True).select_related(
            "order", "order__patient", "order__test_type"
        )
        serializer = LabResultSerializer(results, many=True)
        return Response(serializer.data)

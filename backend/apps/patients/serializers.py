...
from .models import BloodType, Gender, Patient
...
class PatientCreateSerializer(serializers.ModelSerializer):
    ...
    ssn = serializers.CharField(
        write_only=True, required=False, allow_blank=True, help_text="Social Security Number (will be encrypted)"
    )
    ...
    def validate_ssn(self, value):
        ...
        if not ssn_clean.isdigit() or len(ssn_clean) != 9:
            raise serializers.ValidationError("SSN must be 9 digits (format: XXX-XX-XXXX or XXXXXXXXX)")

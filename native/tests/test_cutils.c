...
void test_aes_gcm_encryption() {
    ...
    /* Generate a key */
    int result = cutils_generate_token(key);
    assert(result == CUTILS_SUCCESS);
    
    /* Encrypt */
    result = cutils_aes_gcm_encrypt(
...
void test_sha256() {
    ...
    int result = cutils_sha256((uint8_t*)data, strlen(data), hash);
    assert(result == CUTILS_SUCCESS);
    
    /* Hash should be deterministic */
    uint8_t hash2[CUTILS_SHA256_SIZE];
    result = cutils_sha256((uint8_t*)data, strlen(data), hash2);
    assert(result == CUTILS_SUCCESS);
...
void test_hex_encoding() {
    ...
    int result = cutils_hex_encode(data, sizeof(data), hex);
    assert(result == CUTILS_SUCCESS);
    ...
    uint8_t decoded[16];
    size_t decoded_len;
    result = cutils_hex_decode(hex, decoded, &decoded_len);
    assert(result == CUTILS_SUCCESS);
...
void test_token_generation() {
    ...
    int result = cutils_generate_token(token1);
    assert(result == CUTILS_SUCCESS);
    
    result = cutils_generate_token(token2);
    assert(result == CUTILS_SUCCESS);
